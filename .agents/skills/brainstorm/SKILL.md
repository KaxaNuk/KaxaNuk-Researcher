...
1. **Read the experiment's state.** `BLUEPRINT_N.md`, `BRAINSTORMING_N.md` (the entries so far),
   ...
   `JOURNAL_N.md`.
2. **Contrast with the library and the owner's voice.** What has the owner read that bears on the
   idea, and what does their `Philosophy/` say about it? What argues against it? Is there a simpler
   rival — the cheap version of the idea that the complicated one has to beat?
