...
description: >
  ...
  question that should be answered from Library/, Notes/ and Sources/ rather than from general
  knowledge. It walks Library/INDEX.md and the links between articles before reading anything, and
  cites every claim. Do NOT use it for writing code, for slash commands, or for questions about files
  outside the library.
metadata:
  version: 0.1
---
...
1. **Index first.** Read `Library/INDEX.md` end to end. Note every article whose one-line
...
3. **Read the notes.** Check `Notes/` for the owner's own view on the subject and cite it as theirs,
...
- Read `Notes/Private/` unless the owner names the file.
- Invent a source, a page or a URL.
- Modify `Library/`, `Notes/` or `Sources/` while answering.
