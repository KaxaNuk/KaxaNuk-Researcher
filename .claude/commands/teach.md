---
description: A multi-session tutor grounded in Library/ — interview first, then one lesson per session with a retrieval quiz; state in Output/Teach/<topic>/
argument-hint: "<topic>"
---
...
the owner's explicit ask to write under `Output/Teach/<topic-slug>/`, and nowhere else.
...
1. Ground it in `Library/` by the same walk as `/query`: index first, then links, then articles.
...
Never edit a past session file. Never read `Notes/Private/`. Never write outside
`Output/Teach/<topic-slug>/`.
