...
class TestCopyAuthoredCase:
    ...
    def test_a_named_fixture_is_copied_into_the_case_as_real_files(
        ...
        copied = target / 'contract' / 'read' / 'with-book' / 'fixture-files' / 'Sources' / 'book.md'
        links = [
            path
            for path
            in target.rglob('*')
    ...
    @pytest.mark.skipif(
        os.name == 'nt',
        reason='Windows has no execute bit; the harness runs the scaffold on Linux',
    )
