...
- Not on its own initiative, and not to repair an existing strategy. A file of the template that
  the strategy lacks — lost, or never there because the strategy was made before template 0.11.0 —
  comes back from the template, never from the example, whose copy is filled in: run this skill's
  script from the strategy's root; it copies the template's description, brings nothing of the
  example's to delete, and never overwrites a file.
