...
description: >
  ...
  package, copied by a script and made a git repository, then hand over to `researcher-init` for
...
The home is named after the researcher — `Luna`, not `my-researcher` — and the owner opens it
in a session of its own, or adds it to a strategy's session to bring the library in.

## When to Use

- The owner runs `init-researcher` by name — *init-researcher Luna*, *set up my researcher*.
...
1. **The name and the place.** Ask for whatever is missing through the question tool: the
   ...
   `D:\Research\Luna`, never a deep synced path. Say the full path you will create.
