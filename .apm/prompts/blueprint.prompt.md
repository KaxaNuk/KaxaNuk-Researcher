...
- **`BLUEPRINT_N.md` is already filled** — its *Thesis* section no longer holds the template's
  guidance paragraph. It does not change once written; a new idea is Experiment N+1.
...
and its blockquote. The owner edits and commits it **before writing the rule** — in a commit of
its own, so the history shows the order. Then say what comes next in the order of work: the broad
