...
has been read, so every claim in it points back to a source. `${input:strategy}` is the strategy's
path, left out when the session is open in it.

## Step 1: Read the strategy's own material
